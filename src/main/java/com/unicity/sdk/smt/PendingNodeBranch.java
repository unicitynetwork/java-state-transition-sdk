...
public class PendingNodeBranch implements NodeBranch {
  ...
  public FinalizedNodeBranch finalize(HashAlgorithm hashAlgorithm) {
    ...
    DataHash childrenHash = new DataHasher(hashAlgorithm).update(left.getHash().getData())
        .update(right.getHash().getData()).digest();
    DataHash hash = new DataHasher(hashAlgorithm).update(this.path.toByteArray())
        .update(childrenHash.getData()).digest();
