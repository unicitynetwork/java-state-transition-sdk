...
public class MerkleTreePathStepJson {
    ...
    public static class Serializer extends JsonSerializer<MerkleTreePathStep> {
        @Override
        public void serialize(MerkleTreePathStep value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
            if (value == null) {
                gen.writeNull();
                return;
            }
    ...
    public static class Deserializer extends JsonDeserializer<MerkleTreePathStep> {
        @Override
        public MerkleTreePathStep deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
            ...
            while (p.nextToken() != JsonToken.END_OBJECT) {
                ...
                if (!fields.add(fieldName)) {
                    throw MismatchedInputException.from(p, MerkleTreePathStep.class, String.format("Duplicate field: %s", fieldName));
                }
                    ...
                    switch (fieldName) {
                        case PATH_FIELD:
                            if (p.currentToken() != JsonToken.VALUE_STRING) {
                                throw MismatchedInputException.from(p, MerkleTreePathStep.class, "Expected string value");
                            }
                            path = new BigInteger(p.getValueAsString());
                            break;
                        case SIBLING_FIELD:
                            sibling = p.currentToken() != JsonToken.VALUE_NULL ? p.readValueAs(DataHash.class) : null;
                            break;
                        case BRANCH_FIELD:
                            branch = p.currentToken() != JsonToken.VALUE_NULL ? p.readValueAs(MerkleTreePathStepBranch.class) : null;
                            break;
                        default:
                            p.skipChildren();
                    }
            ...
            if (!missingFields.isEmpty()) {
                throw MismatchedInputException.from(p, MerkleTreePathStep.class, String.format("Missing required fields: %s", missingFields));
            }
