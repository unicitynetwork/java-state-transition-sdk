...
public class SparseMerkleTree {
    ...
    public synchronized void addLeaf(BigInteger path, byte[] data) throws Exception {
        if (path.compareTo(BigInteger.ONE) < 0) {
            throw new Exception("Path must be greater than 0");
        }
    ...
    public synchronized MerkleTreeRootNode calculateRoot() throws Exception {
    ...
    private static Branch buildTree(Branch branch, BigInteger remainingPath, byte[] value) throws Exception {
